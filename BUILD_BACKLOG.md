...
**Status:** Living tracking document. **Baseline as of:** spec v2.30 / research doc v2.24 (2026-09-17).
...
- [x] **Calendar & Appointments UX Overhaul, Phase 4 (Online Patient Self-Booking)** — a second, patient-facing application surface with its own passwordless magic-link auth; self-service book/cancel/reschedule reusing the staff conflict-rule engine; per-type `patient_bookable` opt-in for staff. **Done, v2.30** — see §5a, baseline spec §49. This closes out the 4-phase Calendar & Appointments UX Overhaul roadmap. Pick up waitlist auto-notify, the Phase 3 real-vendor follow-up, or another item from the sections below.
...
- [ ] **Waitlist auto-notify** (deferred from Phase 2/3): when a matching slot opens up, automatically notify queued patients via the same mock notification interface Phase 3 built, instead of only surfacing matches to staff on the appointment detail page.
- [ ] Phase 3 follow-up: swap the mock `send_sms`/`send_email` in `ehr/services/notifications.py` for a real vendor (Twilio/SendGrid or similar) once one is chosen — needs a security/compliance pass (real secrets, real outbound patient contact) before going live with real sends.
...
- [ ] **Portal follow-ups** (not built this round, tracked here): a real email vendor for the magic link (currently mocked/logged, per the Phase 3 follow-up below); patient self-registration (today the portal only authenticates existing chart-matched emails); changing provider/type on a self-service reschedule (time only today); a configurable per-practice self-service cutoff window (hardcoded `PORTAL_SELF_SERVICE_CUTOFF_HOURS` today); a patient-facing view of clinical data (exam history, prescriptions, documents); rate-limiting on the login-link-request endpoint; waitlist self-service (joining/viewing from the portal).
