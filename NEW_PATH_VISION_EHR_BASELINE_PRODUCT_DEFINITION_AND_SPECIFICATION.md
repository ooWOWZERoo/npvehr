...
**Document version:** 2.13 (supersedes v2.12; a documentation-only pass reviewing and reconciling two more uploaded requirements documents — billing/claims/EDI (target-state only, `VISION_EHR_DATA_STANDARDS_RESEARCH.md` §6) and a third document whose proposals mostly conflict with or duplicate already-shipped work (§7, conflicts documented so they aren't reintroduced, plus two compatible candidate ideas) — no code/schema/migration changed; none of this bears on the four go-live prerequisites, which are unchanged from v2.6)
