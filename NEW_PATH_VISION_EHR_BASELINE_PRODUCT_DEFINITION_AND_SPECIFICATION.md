...
**Document version:** 2.11 (supersedes v2.10; builds the Anterior Segment / Dry Eye dashboard documented in `VISION_EHR_DATA_STANDARDS_RESEARCH.md` §5.2 — new `AnteriorSegmentAssessment` table, migration `016_create_anterior_segment_assessments` — and introduces the Visit Focus navigation model needed now that a second Assessment & Plan dashboard shares the exam form; see §12.5b and §36.5 item 14; none of this bears on the four go-live prerequisites, which are unchanged from v2.6)
...
- Insurance eligibility, authorizations, billing, claims, payments, statements, or collections.
