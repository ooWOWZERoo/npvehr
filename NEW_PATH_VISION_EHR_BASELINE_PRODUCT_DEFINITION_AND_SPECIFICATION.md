...
**Document version:** 2.17 (supersedes v2.16; builds the fourth of five clinical dashboards, Binocular Vision & Pediatrics / Vision Therapy — new `BinocularVisionAssessment` table, a fourth Visit Focus chip, and a composer extension; see §12.5f; none of this bears on the four go-live prerequisites, which are unchanged from v2.6)
...
14. **No pre-/post-op co-management data model.** Field-level requirements were reviewed from an uploaded requirements document and documented as target-state only, translated into this app's own conventions (plain columns, no React/async stack, no FHIR mapping yet). See `VISION_EHR_DATA_STANDARDS_RESEARCH.md` §5.5. ~~A structured refractive Assessment & Plan extending §12.6a's work~~ **built in v2.10** — see §12.5a, §12.7a, and `VISION_EHR_DATA_STANDARDS_RESEARCH.md` §5.1. ~~An Anterior Segment / Dry Eye assessment~~ **built in v2.11** — see §12.5b and `VISION_EHR_DATA_STANDARDS_RESEARCH.md` §5.2. ~~Chronic-disease/glaucoma tracking, including longitudinal IOP trending~~ **built in v2.16** — see §12.5e and `VISION_EHR_DATA_STANDARDS_RESEARCH.md` §5.3. ~~Vision-therapy tracking~~ **built in v2.17** — see §12.5f and `VISION_EHR_DATA_STANDARDS_RESEARCH.md` §5.4. Not implemented for §5.5 (Pre/Post-Op); no code, schema, or migration changed for it.
