...
## 50. Waitlist Auto-Notify (v2.31)

### 50.1 Origin

The last deferred item from the Calendar & Appointments UX Overhaul (Phase 2, §47, and Phase 3, §48): Phase 2 shipped waitlist matching that only *surfaced* candidates to staff on an appointment's detail page when it was cancelled; Phase 3 built the mock notification interface those matches needed to actually reach patients. This round wires the two together.

### 50.2 What changed

**`ehr.services.notifications.send_waitlist_opening_notices(db, freed_appointment)`**: called the moment an appointment transitions to `cancelled` (from any other status) -- in every place that can happen: the staff status-change route (`POST /appointments/{id}/status`), the full edit form's status field (`POST /appointments/{id}/edit`), and the patient portal's own self-service cancel (`POST /portal/appointments/{id}/cancel`, §49). It reuses `ehr.services.scheduling.find_matching_waitlist_entries` -- the exact same matching logic Phase 2 already used to surface candidates to staff -- and, for each match, sends a (mock) notice per channel the *waitlisted patient* has opted into, with the same opt-in gating and mock-only posture as Phase 3's `send_appointment_notice`.

**New `WaitlistNotification` model** (migration `029_waitlist_notifications`) -- deliberately a separate table from Phase 3's `AppointmentReminder` rather than reusing it: one freed appointment can match several different waitlist entries/patients, and `AppointmentReminder`'s dedup key (`appointment_id`, `channel`, `kind`) has no room to distinguish which patient a row is for. This table's dedup key is instead (`waitlist_entry_id`, `appointment_id`, `channel`) -- the same entry is never notified twice about the same freed slot, but remains eligible to be notified about a *different* one later, exactly mirroring Phase 3's "dedupe on any prior attempt, not just successful sends" fix (§48.3).

**Staff visibility**: the appointment detail page's existing "Patients Waiting for This Slot" card (§47.2) gained a "Notified" column showing whether each match already got a `sent` notice on this freed appointment. The staff waitlist queue (`GET /appointments/waitlist`, `appointments/waitlist.html`) gained a second "Recent Waitlist Notifications" section -- the last 50 attempted sends across all patients, newest first, mirroring the reminders audit log's (§48.2) format.

### 50.3 Verified

`python3 -m py_compile` on every touched Python file. Fresh-database boot and idempotent re-run against an already-migrated database. Manual end-to-end checks via a live instance: opting a waitlisted patient in for email, adding a matching entry, cancelling the matching appointment, and confirming the mock send fires exactly once (visible both as a "Notified: Yes" on the detail page and a "Sent" row on the waitlist queue's audit section); confirming a reschedule-back-to-scheduled-then-cancel-again does *not* send or log a second notice for the same entry+appointment+channel. One new Playwright test (`test_waitlist_auto_notify_on_cancellation_and_idempotency`) added to the permanent suite; full suite (46 tests) re-run to confirm no regressions.

**A real, pre-existing bug found and fixed along the way**: `patients/form.html` rendered every optional text field (MRN, phone, email, address, insurance info, medical/ocular history, etc.) with `{{ patient.field if patient else '' }}` -- this only guards `patient` itself being falsy (the "new patient" case), not `patient.field` being `None` while `patient` exists, so resubmitting the edit form for any patient with an unset field wrote the *literal string* `"None"` into that column. This was invisible until two different patients both had a null MRN and both got resubmitted through the browser form: the second write collided with the first's now-literal `"None"` value and tripped the MRN-uniqueness conflict check, surfacing this test's own patient-edit step as a 400. Fixed by switching every vulnerable field to `{{ patient.field or '' }}` (Jinja's dot-access on `None`/`Undefined` is falsy, so this is correct whether `patient` itself or just the field is unset) -- `balance_due` already used the equivalent `is not none` guard and was left as-is.

### 50.4 Explicitly not done

No auto-booking or auto-hold of the freed slot for a notified patient -- the notice tells them to call the office, same as the existing "Book" link staff already use; a slot is still first-come-first-served among however many patients get notified. No "fulfilled" status automation (still a manual field, per §47.2's original scope note). No notification on a *reschedule's* vacated slot (only cancellation triggers this, matching Phase 2's original scoping rationale: a reschedule's old slot is a one-off signal this app has no toast/queue mechanism to carry, unlike a cancellation which the appointment's own current, queryable state already represents). No rate-limiting or batching of notices when many entries match one freed slot. This closes out the last item explicitly tracked in `BUILD_BACKLOG.md` §5a's Phase 2/3 follow-up list; the Phase 3 real-vendor swap and the Phase 4 portal follow-ups (§49.4) remain separately tracked.

**Version 2.31 change log (relative to v2.30) — Waitlist Auto-Notify:**

| Area | Change |
| --- | --- |
| New capability | `ehr.services.notifications.send_waitlist_opening_notices()`, called on every appointment cancellation path (staff status change, staff edit form, patient portal cancel) -- sends a (mock) notice to every matching active waitlist entry, gated per-channel on that patient's own opt-in. New `WaitlistNotification` model + migration `029_waitlist_notifications`, deduped on (waitlist_entry_id, appointment_id, channel). See §50.2. |
| New capability | Staff visibility: a "Notified" column on the appointment detail page's waitlist-matches card, and a "Recent Waitlist Notifications" audit section on the staff waitlist queue page. See §50.2. |
...
## 52. Scheduling Slot/Duration Reconciliation (v2.33)

### 52.1 Origin

User question: "how do you change the calendar time slots (20, 30, 40 minutes)" surfaced a real conceptual gap once explored further -- appointment duration was already correctly type-driven (`AppointmentTypeVersion.new_duration_minutes`/`established_duration_minutes`, editable per type, §admin scheduling), but the *search step* used to walk the day for candidate start times (`SLOT_UNIT_MINUTES = 5`) was hardcoded and global, with no practice-configurable "offered granularity" concept at all. Investigating this surfaced a second, more consequential gap: `find_open_slots` never checked room/resource conflicts, only provider availability -- so a slot could be *offered* (on the calendar, in the patient portal) that then failed at actual booking with a resource conflict `_apply_scheduling_rules` caught separately. This round fixes both, plus threads the fix through the FullCalendar board's visual grid and drag/resize snapping.

### 52.2 Design decisions

Confirmed before building: **(1)** slot granularity is a **per-provider override** on top of a practice-wide default (not a single global-only setting), since different providers can reasonably want different offering cadences; **(2)** staff drag-and-drop rescheduling on the calendar **snaps to the same granularity** offered for new bookings, so the board never implies finer control than a fresh booking would actually get.

The core principle carried through the implementation: **conflict-checking always runs at `SLOT_UNIT_MINUTES` (5-minute) resolution, unconditionally** -- granularity is purely a filter applied *after* the fine-grained, always-correct safe-slot list is computed, narrowing which of those already-safe times get offered. A coarser granularity can only ever remove options from what's shown; it can never hide a real conflict or manufacture an unsafe time. This directly answers the scenario in the question: an exam type that needs more time than a given slot size was never actually at risk of being squeezed in (duration/buffer conflict-checking was always exact, not slot-box-based) -- what was missing was purely the offering-cadence configurability and the resource-conflict blind spot below.

### 52.3 What changed

**Configurable slot granularity**: new `SchedulingSettings` singleton (migration `031_slot_granularity`, one row, `id=1`, `default_slot_granularity_minutes` defaulting to `5` so no existing deployment's behavior changes until an admin explicitly sets it coarser) plus a new, nullable `Provider.slot_granularity_minutes` override column (same migration). `ehr.services.scheduling.get_effective_slot_granularity(db, provider_id)` resolves the two (provider override, else the practice default) into the value `find_open_slots` filters its fine-grained result against. Staff manage both from a new section on the existing Provider Availability admin page (`admin/scheduling/provider_availability.html` -- the natural home, since this app has no separate provider-CRUD page to add a "per-provider setting" onto): a practice-default selector plus a per-provider override row (5/10/15/20/30/60-minute choices, blank = inherit).

**Resource-conflict-aware slot search**: `find_open_slots` now takes an optional `version` parameter and, when given, runs each fine-grained candidate through `plan_resource_requirements`/`find_resource_conflict`/`find_resource_blocked_exception` -- the exact same resource-availability check `_apply_scheduling_rules` already applies at actual booking time. Every caller (`GET /appointments/availability`, the patient portal's `_slot_search_context`) now passes `version`, so "shown as open" and "actually bookable" can no longer disagree.

**Calendar grid and drag/resize snapping** (`appointments/board.html`): the FullCalendar board's `slotDuration`/`snapDuration` now reflect each provider column's effective granularity (`GRANULARITY_BY_PROVIDER`, computed server-side per the design decision above) instead of FullCalendar's own default; single-calendar (non-Providers) view uses the currently-filtered provider's granularity, or the practice default when no provider filter is set. A known limitation of this first pass: changing the provider filter afterward re-fetches events but doesn't recreate the calendar, so the grid/snap granularity reflects the filter as of page load, not a live change.

### 52.4 Verified

`python3 -m py_compile` on every touched Python file. Fresh-database boot and idempotent re-run against an already-migrated database. Manual end-to-end checks via a live instance: the practice default and a per-provider override each independently and correctly filter the offered-times list (confirmed at 5/10/30-minute granularities, with the per-provider override taking precedence over the practice default); direct reproduction of the resource-conflict reconciliation bug -- booked a resource-requiring type for one provider, confirmed a *different* provider's availability search correctly stopped offering the same exact time (previously it would have, since the check only considered the provider), and confirmed attempting to book it anyway is rejected with the same conflict message the full form already gives; the board and single-day/week/month calendar pages all load with the new per-provider granularity data correctly embedded. One new Playwright test (`test_slot_granularity_and_resource_conflict_reconciliation`) added to the permanent suite; full suite (47 tests) re-run twice to confirm no regressions.

**A second, separate pre-existing gap found (not fixed) while writing this round's test**: `admin_scheduling.py`'s `publish_new_version` carries forward a type's color rules onto the new version but not its resource requirements (`AppointmentTypeResourceRequirement` rows stay attached to the now-inactive previous version) -- so republishing a type that requires a room/resource silently drops that requirement from the new version. Logged to `BUILD_BACKLOG.md` rather than fixed here, since it's outside this round's scope and deserves its own verification pass (confirming the intended behavior -- should a republish always carry every resource requirement forward, or is there a legitimate case for changing them at that point?).

### 52.5 Explicitly not done

No change to the underlying `SLOT_UNIT_MINUTES = 5` conflict-checking resolution itself -- it stays fixed and always-correct; granularity is presentation-only. No live-update of the board's grid/snap granularity when the provider filter changes without a full page reload (documented limitation above). No granularity control in the patient portal's own UI (the portal's server-rendered slot buttons already reflect the effective granularity automatically, with no separate JS grid to update). The `publish_new_version` resource-requirements gap found above is tracked, not fixed, in `BUILD_BACKLOG.md`.

**Version 2.33 change log (relative to v2.32) — Scheduling Slot/Duration Reconciliation:**

| Area | Change |
| --- | --- |
| New capability | Configurable slot granularity: practice-wide default (`SchedulingSettings`, migration `031_slot_granularity`) with an optional per-provider override (`Provider.slot_granularity_minutes`), managed from a new section on `admin/scheduling/provider_availability.html`. See §52.3. |
| Bug fix | `find_open_slots` never checked room/resource conflicts, only provider availability, so a slot could be offered that then failed at actual booking -- fixed by threading the appointment type `version` through to the same resource-conflict checks `_apply_scheduling_rules` already uses. See §52.3. |
| Updated | `appointments/board.html`'s FullCalendar grid/drag/resize snapping now reflects each provider's effective granularity instead of a hardcoded default. See §52.3. |
...
## 54. Chief-Complaint Triage + E/M-Level Suggestion (v2.35, Phase 1 of 4)

### 54.1 Origin

User request describing a real-world optometry-EHR workflow: scan a chief complaint for keywords to suggest the right exam type; auto-suggest an E/M level (99212/99213/99214) from documented MDM complexity (problems addressed, data reviewed, risk), with a hard rule that any prescription-drug-management language automatically meets Moderate risk; map visits and tests to CPT codes; route patients through two billing flows (vision-plan vs. medical-insurance, with split billing); track diagnostic orders with a real lifecycle; and run a "look-back" clinical alert engine flagging overdue chronic-disease testing. Investigation found none of the underlying infrastructure exists today (no CPT/billing-code column anywhere, no orders table beyond a thin scheduling-only link, no structured insurance-plan-type field, no `EyeExam`-to-`Appointment` link at all), and this app is explicitly documented as not for use with real patient data, with the entire billing/claims domain already gated in `BUILD_BACKLOG.md` behind a real clearinghouse relationship and compliance/legal review. The request was planned as four independently-shippable phases; this round builds Phase 1 only -- chief-complaint triage and E/M-level suggestion, both pure client-side decision support, never transmitted or submitted anywhere. Phases 2-4 (CPT mapping + two-flow billing preview, diagnostic order tracking, and the look-back alert engine) are scoped in the planning document but not started.

### 54.2 What changed

**Schema**: five new nullable `VARCHAR` columns on `EyeExam` (migration `033_exam_type_and_em_suggestions`): `suggested_exam_type`/`exam_type_confirmed` and `suggested_em_code`/`suggested_em_rationale`/`em_code_confirmed`. Following this app's established `follow_up_weeks`/`follow_up_unit` convention, `suggested_*` holds what the client-side scan/scoring produced and `*_confirmed` holds what the clinician actually accepted or typed over it, so a confirmed value is never silently recomputed out from under them.

**Chief-complaint keyword triage** (`exams/form.html`): a new `EXAM_TYPE_BY_KEYWORD` lookup, an ordered array of `{category, keywords}` entries checked most-urgent-first (Emergent/Urgent Medical → Medical Established (Intermediate) → Comprehensive New/Established → Routine Vision) against a lowercased substring match on the chief complaint. The first category with any keyword hit wins, so a complaint mentioning both an urgent symptom and a routine reason resolves to the more clinically significant category. Recomputed on every chief-complaint keystroke into a visible `exam_type_confirmed` `<select>`, same "edited flag" pattern as every other auto-suggested field on this form.

**E/M-level (MDM) suggestion**: three factors, each scored into the same three tiers the user specified (Straightforward/Low/Moderate, mapping directly to 99212/99213/99214) --
- *Problems*: count of checked "Problems Addressed" boxes plus entered diagnosis codes (0 → Straightforward, 1 → Low, 2+ → Moderate). An explicit heuristic, not the full CMS table (it can't distinguish "stable chronic" from "acute complicated"), so it never reasons above Moderate.
- *Data*: a new "Tests Ordered/Reviewed This Visit" checkbox group (OCT/Visual Field/Fundus Photos/Topography) -- an explicit Phase 1 proxy for "amount/complexity of data reviewed" since no real diagnostic-order tracking exists yet (Phase 2 will supersede it). Not itself saved to the chart.
- *Risk*: Straightforward by default, forced straight to Moderate the instant prescription-drug-management language (`RX_MANAGEMENT_KEYWORDS` -- refill/discontinue/dose-change/named glaucoma drops, etc.) appears anywhere in the chief complaint or the composed plan, per the user's explicit hard-trigger rule.

The overall suggestion counts how many of the three factors reach Moderate: none → 99212, exactly one → 99213, two or more → 99214. This is deliberately looser than the textbook CMS "2 of 3 elements meet the same tier" rule (which would let a single Moderate factor get outvoted back down to 99212 by two Straightforward ones) -- that stricter version would make the Rx-management hard trigger toothless, since risk can only ever be Straightforward or Moderate in this heuristic, never Low. A one-line rationale (e.g. "Moderate: 2 problems addressed + Rx management noted") is generated alongside the code and shown next to the suggested-E/M `<select>`.

**A real, previously-unnoticed bug fixed while wiring this in**: a `<select>` element fires both `input` and `change` on a user selection (in that order), and this form's generic "recompute on any edit" wiring listens for both events on every tracked field. The two new suggestion fields' "has the clinician overridden this" flag was originally set only on `change` -- so the generic `input`-triggered recompute fired first (seeing the flag still `false`) and silently reverted the clinician's own selection back to the suggestion before `change` ever had a chance to freeze it. Fixed by setting the edited flag on `input` too. (This same latent pattern exists for `follow_up_unit`, added in v2.34, but its own auto-suggestion only fires under narrow conditions that hadn't triggered it in testing; not touched in this round since it's outside this round's scope, and logged to `BUILD_BACKLOG.md`.)

**Display**: `exams/detail.html` gained "Exam Type" and "E/M Code" rows (the latter showing the confirmed code plus its rationale in muted text).

### 54.3 Verified

`python3 -m py_compile` on every touched Python file. Fresh-database boot and idempotent re-run confirm the new columns. Manual verification via a live instance and a real browser: keyword triage correctly resolves both single-category and mixed urgent/routine chief complaints; the two-of-three-factors-moderate → 99214 case and the one-factor-alone → 99213 case (proving the Rx trigger has real effect on its own) both compute correctly; manual overrides of both suggestions survive further chief-complaint edits after the `input`/`change` fix above. One new Playwright test (`test_chief_complaint_triage_and_em_suggestion`) covers all of the above plus persistence to the exam detail page; full suite (49 tests) re-run to confirm no regressions.

### 54.4 Explicitly not done

No CPT/billing code of any kind is generated, stored against a claim, or transmitted anywhere -- this remains pure staff-facing decision support (this app has zero billing/claims infrastructure, and the domain is explicitly gated in `BUILD_BACKLOG.md`). No real diagnostic-order data source for the "data reviewed" MDM factor (Phase 2/3 of the larger plan). No attempt to reproduce the full CMS 2021 E/M table's nuance (stable vs. progressing chronic conditions, acute-complicated vs. acute-uncomplicated) -- this is a heuristic aid a clinician must still confirm, never a coding engine. The same `input`/`change` edited-flag ordering issue on `follow_up_unit` (v2.34) is not fixed in this round. Phases 2-4 of the larger plan (CPT mapping + two-flow billing preview, diagnostic order tracking, look-back clinical alert engine) are not started. None of this bears on the four go-live prerequisites (§38.6), which are unchanged from v2.6.

**Version 2.35 change log (relative to v2.34) — Chief-Complaint Triage + E/M-Level Suggestion:**

| Area | Change |
| --- | --- |
| New capability | Chief-complaint keyword scan suggests an exam type (Emergent/Urgent Medical, Medical Established, Comprehensive New/Established, Routine Vision); a 3-factor MDM heuristic suggests an E/M code (99212/99213/99214) with a one-line rationale, hard-triggered to at least Moderate risk by prescription-drug-management language. Both are decision support only, editable/overridable, never transmitted. New `EyeExam` columns via migration `033_exam_type_and_em_suggestions`. See §54.2. |
| Bug fix | A `<select>`'s `input`-before-`change` event order could let this form's generic recompute wiring silently revert a clinician's manual selection on the two new suggestion fields; fixed by also setting each field's edited flag on `input`. See §54.2. |
| Updated | `ehr/models/database.py`, `ehr/db/migrations.py`, `ehr/routes/exams.py`, `ehr/templates/exams/{form,detail}.html`. `tests/test_smoke.py` gained one new test. |
| Found, not fixed (tracked) | The same `input`/`change` edited-flag ordering issue also latently affects `follow_up_unit` (v2.34); not triggered by existing tests and out of this round's scope -- logged to `BUILD_BACKLOG.md`. |
| Explicitly not done | No CPT/billing code generation, storage, or transmission of any kind (staff-facing decision support only). No real diagnostic-order data source yet for the "data reviewed" factor. Phases 2-4 of the larger 4-phase plan (CPT mapping/billing preview, diagnostic orders, look-back alerts) not started. None of this bears on the four go-live prerequisites (§38.6), which are unchanged from v2.6. |

## 55. CPT Mapping + Two-Flow Billing Preview (v2.36, Phase 2 of 4)

### 55.1 Origin

Phase 2 of the 4-phase plan scoped in §54.1 (BUILD_BACKLOG.md 0a): map visits and diagnostic tests to CPT codes, and route patients through two billing flows -- routine vision-plan visits (refraction bundled with the exam code onto one vision-plan claim) versus medical-insurance visits (refraction non-covered, billed separately to the patient as Claim 2 alongside a medical Claim 1 of E/M + testing CPTs + diagnoses). As with Phase 1, this stays strictly staff-facing decision support -- a read-only, on-screen preview, never a submitted or transmitted claim -- since this app has no billing/claims infrastructure at all and that domain remains explicitly gated in `BUILD_BACKLOG.md`.

### 55.2 What changed

**Schema** (migration `034_cpt_and_visit_flow`, plus data-seeding migration `035_seed_cpt_codes`): a new curated `cpt_codes` catalog table (13 codes: 92004/92014/92012 exam, 92015 refraction, and the testing codes named in the original request -- 92133/92134/92250/92083/92025/76514/92020/76512/76513/92132/92285), the same "narrow lookup, not a real terminology server" posture already established for ICD-10. A new nullable `DiagnosticTest.cpt_code` column, statically mapped for five of the seven seeded catalog tests (OCT→92134, OPTOS→92250, VF→92083, CORNEAL_ANALYZER→92025, MEIBOGRAPHY→92285; ERG and TearLab osmolarity left null -- no single CPT in this curated set cleanly covers either). A new `patient_insurance_plans` table (`plan_category`: 'vision'/'medical', payer/member/group fields, `is_active`) -- a table, not a single field on `Patient`, since the medical flow still bills refraction to the patient even when a vision plan is also on file, so a patient needs to be able to carry both simultaneously; additive, the existing flat `Patient.insurance_provider`/`insurance_id` fields are untouched. A new nullable `EyeExam.appointment_id` FK -- previously `EyeExam` and `Appointment` were entirely unconnected; now an exam started from an appointment's "Start Exam" link carries that link forward as a hidden form field. New `Appointment.visit_flow`/`visit_flow_source` columns (`'vision'`/`'medical'`, `'automatic'`/`'manual_override'` -- mirroring the existing `patient_relationship_source` idiom exactly).

**Check-in flow selection**: a new "Check-In / Visit Flow" section on the appointment detail page (`POST /appointments/{id}/visit-flow`). `ehr.services.cpt_mapper.suggest_visit_flow` auto-suggests 'medical' when the patient has an active medical insurance plan on file (the more constrained flow, since refraction must still be billed separately even alongside a vision plan), otherwise defaults to 'vision'. Staff confirm or override via a dropdown; confirming records whether the stored value matches the suggestion (`'automatic'`) or was changed (`'manual_override'`).

**CPT mapping** (`ehr/services/cpt_mapper.py`, new, mirroring `ap_composer.py`'s pure-computation-module convention but reading the database directly, since the billing preview is server-rendered on page load rather than a live-typing client-side suggestion): `compute_cpt_summary(db, exam)` returns `None` when the exam has no linked appointment (a walk-in entered directly, or any pre-Phase-2 exam) -- there's no visit-flow context to build a preview from. Otherwise it resolves the visit-level exam code from patient relationship (92004 new / 92014 established, regardless of flow), the testing codes from that appointment's active `AppointmentTest` rows joined to their `DiagnosticTest.cpt_code`, the E/M code from Phase 1's `em_code_confirmed`/`suggested_em_code`, the diagnosis codes already on the exam, and the patient's active vision/medical payer names.

**Insurance plan management**: the existing (previously read-only) Insurance tab (`patients/insurance_tab.html`) gained an "Insurance Plans" card listing/adding/deactivating `PatientInsurancePlan` rows, feeding the check-in suggestion above.

**Billing preview**: a new read-only card on the exam detail page, explicitly labeled "staff reference only, not a submitted claim" with a closing "Preview only -- no claim has been filed or transmitted" line. A medical-flow exam renders two blocks (Claim 1: E/M + exam code + testing codes + diagnoses, to the medical payer; Claim 2: 92015 to patient responsibility/ABN); a vision-flow exam renders one bundled block (92015 + exam code + testing codes, to the vision payer); an exam not yet checked in (`visit_flow` still unset) shows a prompt linking back to the appointment page instead of a preview. No PDF export, no submit action, and no `Invoice`/`Claim` row is ever created -- every render is computed fresh from `compute_cpt_summary`.

### 55.3 Verified

`python3 -m py_compile` on every touched Python file. Fresh-database boot and idempotent re-run confirm both new tables, the CPT catalog seed data, the `diagnostic_tests.cpt_code` backfill, and the new columns on `eye_exams`/`appointments`. Manual verification via a live instance and a real browser: adding a medical plan flips the check-in suggestion from 'vision' to 'medical'; confirming a flow and starting an exam from that appointment correctly carries `appointment_id` through to the saved exam; the medical-flow billing preview renders the correct two-claim split with the payer name and testing codes; the vision-flow preview renders the single bundled block; a walk-in exam with no linked appointment shows no billing card at all (backward compatible). One new Playwright test (`test_cpt_mapping_and_two_flow_billing_preview`) covers all of the above end to end; full suite (50 tests) re-run to confirm no regressions.

### 55.4 Explicitly not done

No claim is ever generated, stored, or transmitted -- no `Invoice`/`Claim` table exists, and the preview is computed fresh on every page load, never persisted. No fee-schedule/dollar-amount data (CPT codes and descriptions only). No CPT for the two seeded tests genuinely ambiguous in this curated set (ERG, TearLab). No automatic linkage of a *pre-existing* `AppointmentTest`/order booking flow to a `DiagnosticOrder` lifecycle -- that's Phase 3. No look-back/interval alerting -- Phase 4. None of this bears on the four go-live prerequisites (§38.6), which are unchanged from v2.6.

**Version 2.36 change log (relative to v2.35) — CPT Mapping + Two-Flow Billing Preview:**

| Area | Change |
| --- | --- |
| New capability | Curated `cpt_codes` catalog + `DiagnosticTest.cpt_code` mapping (migrations `034_cpt_and_visit_flow`/`035_seed_cpt_codes`); `PatientInsurancePlan` table and management UI on the Insurance tab; check-in "Visit Flow" selection (`Appointment.visit_flow`/`visit_flow_source`) with an auto-suggestion from insurance plans on file; `EyeExam.appointment_id` finally links exams back to their appointment. See §55.2. |
| New capability | Read-only "Billing Preview" card on the exam detail page (`ehr.services.cpt_mapper`) -- a medical-flow split (Claim 1 medical + Claim 2 patient-responsibility refraction) or a vision-flow bundled block, explicitly labeled as a preview, never a submitted claim. See §55.2. |
| Updated | `ehr/models/database.py`, `ehr/db/migrations.py`, `ehr/routes/{appointments,exams,patients}.py`, `ehr/templates/appointments/detail.html`, `ehr/templates/exams/{form,detail}.html`, `ehr/templates/patients/insurance_tab.html`. New `ehr/services/cpt_mapper.py`. `tests/test_smoke.py` gained one new test. |
| Explicitly not done | No claim generation, storage, or transmission of any kind -- no Invoice/Claim table, nothing persisted beyond the underlying CPT/insurance/flow data itself. No fee-schedule/dollar amounts. Phases 3-4 (diagnostic order tracking, look-back alerts) not started. None of this bears on the four go-live prerequisites (§38.6), which are unchanged from v2.6. |

## 56. Diagnostic Order Tracking (v2.37, Phase 3 of 4)

### 56.1 Origin

Phase 3 of the 4-phase plan (BUILD_BACKLOG.md 0a): a real diagnostic-test order lifecycle, so a clinician's order persists as trackable, patient-scoped state rather than only a free-text plan-line note -- the foundation Phase 4's look-back alert engine will query against for "outstanding order" alerts.

### 56.2 What changed

**Schema**: a new `diagnostic_orders` table (migration `036_create_diagnostic_orders`), patient-scoped (not appointment-scoped -- an order can be placed at one visit and fulfilled at a later one), with a plain-string lifecycle `status` (`ordered`/`scheduled`/`in_progress`/`completed`/`cancelled`, matching this app's existing convention of validating transitions in the service/route layer rather than a DB enum), `ordered_exam_id`/`completed_exam_id` (both nullable -- an order can originate or be resolved outside any specific exam), `result_summary` (free text; real device/DICOM result ingestion stays out of scope), and `cancelled_reason`. Indexed on `(patient_id, status)` and `(diagnostic_test_id, status)` -- the exact two lookups this phase and Phase 4 both need. Deliberately coexists with, rather than replaces, the pre-existing `AppointmentTest` table: that one answers "does this specific calendar appointment need scheduling-duration/color-rule credit for a test" (a concern `ehr.services.scheduling` depends on structurally), while `DiagnosticOrder` answers "what has a clinician ordered for this patient, and is it done yet" -- a clinical concern with its own lifecycle, unrelated to any one appointment.

Two new catalog rows (migration `037_seed_gonioscopy_pachymetry`): the Glaucoma dashboard's "Diagnostic Orders" checkboxes have offered Gonioscopy and Pachymetry since before any CPT-mapping catalog existed, but neither was ever seeded into `diagnostic_tests` -- both now exist with their CPT codes (92020, 76514) filled in via the same static-mapping mechanism v2.36 introduced.

**Transition validation** (`ehr/services/diagnostic_orders.py`, new): a small `ALLOWED_TRANSITIONS` dict plus `transition()`, mirroring `ap_composer.py`/`cpt_mapper.py`'s thin-service-module convention -- takes an already-loaded order and mutates it in place (stamping `completed_at`/`completed_by_user_id`/`result_summary` or `cancelled_at`/`cancelled_reason` as appropriate), raising `ValueError` on an invalid edge (including a no-op re-transition to the same terminal state).

**Order creation**: the Glaucoma dashboard's "Diagnostic Orders" checkboxes (`exams/form.html`) now submit a `diagnostic_tests` catalog code (`OCT`/`VF`/`GONIOSCOPY`/`PACHYMETRY`) as their `value`, carrying the previous human-readable text (`"OCT RNFL"`, etc.) in a new `data-label` attribute instead -- needed so `ehr/routes/exams.py`'s `create_exam` can look up a real `DiagnosticTest` row and create a `DiagnosticOrder` (`status='ordered'`) per checked box, while the live-composed Plan text and the saved `GlaucomaTracking.diagnostic_orders` free-text field both still read the human-readable label (a small `GT_DIAGNOSTIC_ORDER_LABELS` map in `exams.py`, kept in sync by hand with the checkboxes' `data-label` attributes -- both are small and rarely change). Unknown/deactivated codes are silently skipped rather than failing the whole exam save.

**Completion/cancellation**: new `POST /patients/{id}/orders/{order_id}/complete` (optional free-text `result_summary`) and `.../cancel` (optional `cancelled_reason`) routes, added directly to `ehr/routes/patients.py` (consistent with how the Phase 2 insurance-plan routes were added there, rather than a new router file). Resolution is intentionally a standalone "Mark Complete"/"Cancel" action this round, not deep-linked into the originating Visit Focus dashboard section -- a generic `DiagnosticTest` (e.g. TearLab, ERG) has no natural section to auto-expand, and building that mapping is speculative scope creep; logged below as a named future refinement.

**Surfacing**: a new "Pending Diagnostic Orders" card on the patient workspace Overview tab (any order still `ordered`/`scheduled`/`in_progress`), reusing the existing `.badge`/`.badge-{status}` CSS convention (new `.badge-ordered` variant added alongside the pre-existing appointment-status colors). A header-level pending-order count badge (mentioned as a nice-to-have in the original plan, alongside the existing allergy-flag indicator) was not built this round -- it would require computing the count in `_workspace_ctx`, used by every workspace-tab route, a broader change than this slice needed; logged below.

### 56.3 Verified

`python3 -m py_compile` on every touched Python file. Fresh-database boot and idempotent re-run confirm the new table, its indexes, and the two new catalog rows with their CPT codes. Manual verification via a live instance and a real browser: checking two Diagnostic Orders boxes on the New Exam form creates two real `DiagnosticOrder` rows and composes Plan/GlaucomaTracking text with the readable labels (not the raw codes now submitted); both orders appear on the patient's Pending Diagnostic Orders card; marking one complete removes only that one from the list; cancelling the other empties the card entirely (it stops rendering once no orders are pending). One new Playwright test (`test_diagnostic_order_created_from_exam_and_resolved`) covers exam-to-order creation, label correctness in both the Plan text and the saved dashboard field, and both terminal actions; full suite (51 tests) re-run to confirm no regressions.

### 56.4 Explicitly not done

No CPT attachment happens at the order level in this phase (Phase 2's `AppointmentTest`→`DiagnosticTest.cpt_code` join remains the only CPT-mapping path; a `DiagnosticOrder` references the same `DiagnosticTest` catalog row, so the mapping is available if a future round wants to read it from there). No real device/DICOM result ingestion -- `result_summary` stays free text. No deep-linking of order completion into a specific Visit Focus dashboard section (documented above as a named future refinement). No header-level pending-order count badge. No look-back/interval-based alerting that *consumes* this phase's data -- that's Phase 4. None of this bears on the four go-live prerequisites (§38.6), which are unchanged from v2.6.

**Version 2.37 change log (relative to v2.36) — Diagnostic Order Tracking:**

| Area | Change |
| --- | --- |
| New capability | Real `DiagnosticOrder` lifecycle table (`ordered`/`scheduled`/`in_progress`/`completed`/`cancelled`, migration `036_create_diagnostic_orders`) with transition validation in `ehr/services/diagnostic_orders.py`; wired to the Glaucoma dashboard's existing Diagnostic Orders checkboxes; a "Pending Diagnostic Orders" card on the patient overview tab with one-click Mark Complete/Cancel actions. See §56.2. |
| New catalog data | `GONIOSCOPY`/`PACHYMETRY` diagnostic-test rows and their CPT codes (migration `037_seed_gonioscopy_pachymetry`) -- these tests were offered on the exam form since before any catalog/CPT infrastructure existed, but were never actually seeded. See §56.2. |
| Updated | `ehr/models/database.py`, `ehr/db/migrations.py`, `ehr/routes/{exams,patients}.py`, `ehr/templates/exams/form.html`, `ehr/templates/patients/overview.html`, `ehr/static/css/app.css`. New `ehr/services/diagnostic_orders.py`. `tests/test_smoke.py` gained one new test. |
| Explicitly not done | No CPT attachment at the order level (Phase 2's AppointmentTest-based mapping is unaffected). No device/DICOM result ingestion. No deep-linked order-completion UI or header-level pending-order count badge (both logged as future refinements). Phase 4 (look-back alerts) not started. None of this bears on the four go-live prerequisites (§38.6), which are unchanged from v2.6. |

## 57. Look-Back & Clinical Alert Engine (v2.38, Phase 4 of 4)

### 57.1 Origin

Phase 4, the last phase of the 4-phase plan (BUILD_BACKLOG.md 0a): an automated clinical audit engine flagging overdue chronic-disease testing (e.g. glaucoma needing a visual field/OCT within 6-12 months) and orders still outstanding from a prior visit -- built entirely on Phase 3's `diagnostic_orders` table and the existing Problem list, with no new schema. Explicitly required by the original request to avoid "alert fatigue": ambient, glanceable banners, never a blocking modal a clinician has to click through.

### 57.2 What changed

**No new schema.** A new `ehr/services/lookback_alerts.py` runs two checks purely as queries: (1) any `DiagnosticOrder` still `ordered`/`scheduled`/`in_progress` for the patient, and (2) for each Active `Problem` matching a small curated `CONDITION_PROFILES` list (the same "narrow hardcoded lookup, not a real rules engine" posture already used for ICD-10/CPT), whether the most recent *completed* order for each of that condition's required tests falls within the required interval. Two profiles seeded to start: Glaucoma (`H40.` prefix, VF+OCT, 365 days default / 182 days when `severity_or_stage` mentions "moderate" or "severe") and Plaquenil retinal-toxicity monitoring (`Z79.899`, VF+OCT, 365 days flat). `DiagnosticOrder.completed_at` is the sole source of truth for "when was this test last done" -- `GlaucomaTracking`'s own `follow_up_interval`/`diagnostic_orders` fields are free text tied only to that exam's date, not a structured, queryable signal, so they stay documentation-only and are never cross-checked against this engine (avoiding two "last done" clocks that could silently disagree). Evaluated synchronously on every relevant page load, not via a background job -- this app has no generic scheduled-job runner beyond one specific cron-secret-gated reminder endpoint, so request-time evaluation is the correct fit, and the query cost (a handful of indexed lookups per patient) is cheap enough for that.

**A real bug caught and fixed during manual verification, before it shipped**: the first working version showed an "interval due" alert for a test *at the same time* as an "outstanding order" alert for that identical test once it had just been ordered -- exactly the redundant noise the "avoid alert fatigue" requirement was written to prevent. Fixed by excluding any test with an outstanding order from the interval-due check; ordering it is already the correct response, so the interval-due nag disappears until/unless that order is cancelled.

**Rendering**: a new reusable partial (`patients/_lookback_alerts.html`), included on both the patient workspace Overview tab and the New Exam form header (the two places a clinician is about to act on a patient), reusing the existing two-tier `.alert-warning` (outstanding order)/`.alert-info` (interval due) palette rather than inventing new colors -- an outstanding-order banner reads "Outstanding order: {test} from {date} is pending," an interval-due banner reads "{Protocol}: {test} due (last done: N months ago)" or "(never completed)."

**One-click resolution**: each banner carries its own inline form. An outstanding-order banner's "Mark Complete" button posts directly to Phase 3's existing `.../orders/{id}/complete` route. An interval-due banner's "Order Now" button posts to a new `POST /patients/{id}/orders/quick-order` route (`ehr/routes/patients.py`) that creates a `DiagnosticOrder` directly with no exam context (`ordered_exam_id` is nullable for exactly this case) -- the only new mutation endpoint this phase needed, since interval-due alerts have no pre-existing order to act on.

### 57.3 Verified

`python3 -m py_compile` on every touched Python file -- no migration/idempotency check applies since no schema changed. Manual verification via a live instance and a real browser: an Active glaucoma Problem with no completed VF/OCT shows both interval-due banners on the patient overview and the New Exam form; clicking "Order Now" swaps that one banner for an outstanding-order warning (confirmed the redundant-alert bug above and its fix, before either landed in the committed version); clicking "Mark Complete" clears both banners for that test, leaving only the genuinely-still-due one. One new Playwright test (`test_lookback_alerts_interval_due_and_outstanding_order`) covers all of the above end to end, using a freshly-created patient rather than any seeded one -- investigation found "Brown" (alphabetically first by last name) is the default, unselected patient on roughly twenty other tests' unqualified `/exams/new` visits, making it the single most cross-test-contaminated patient in the whole suite and an unsafe choice for an assertion this precise; full suite (52 tests) re-run to confirm no regressions.

### 57.4 Explicitly not done

No configurable/admin-managed rules matrix -- `CONDITION_PROFILES` is a small hardcoded list, extended by editing code, matching this app's established ICD-10/CPT convention. No background/scheduled evaluation -- alerts are computed fresh on the page loads that show them. No additional condition profiles beyond the two the user specifically named (glaucoma, Plaquenil) -- AMD/diabetic-retinopathy/keratoconus profiles from the original request's fuller matrix are a straightforward follow-up in the same shape, not built this round. No deep-linking of an interval-due "Order Now" action to a specific Visit Focus dashboard section. This closes out the 4-phase chief-complaint/CPT/billing-flow plan (BUILD_BACKLOG.md 0a) started in v2.35 -- Phases 1-4 are all done, with a handful of named follow-up refinements tracked there. None of this bears on the four go-live prerequisites (§38.6), which are unchanged from v2.6.

**Version 2.38 change log (relative to v2.37) — Look-Back & Clinical Alert Engine:**

| Area | Change |
| --- | --- |
| New capability | `ehr.services.lookback_alerts` evaluates two checks per patient (outstanding diagnostic orders; chronic-condition testing-interval compliance against a curated `CONDITION_PROFILES` list) with no new schema, rendering ambient `.alert-warning`/`.alert-info` banners (never a blocking modal) on the patient overview tab and the New Exam form header, each with a one-click resolution action. New `POST /patients/{id}/orders/quick-order` route for the interval-due alert's "Order Now" action. See §57.2. |
| Bug fix (caught pre-ship) | An interval-due alert and an outstanding-order alert for the same test would otherwise both show at once right after ordering it -- fixed by suppressing the interval-due check for any test with an order already outstanding. See §57.2. |
| Updated | `ehr/routes/{exams,patients}.py`, `ehr/templates/exams/form.html`, `ehr/templates/patients/overview.html`. New `ehr/services/lookback_alerts.py`, `ehr/templates/patients/_lookback_alerts.html`. `tests/test_smoke.py` gained one new test. |
| Explicitly not done | No configurable rules matrix (small hardcoded list, same convention as ICD-10/CPT). No background/scheduled evaluation. No profiles beyond glaucoma/Plaquenil (AMD/DR/keratoconus are a straightforward follow-up). No deep-linked "Order Now" action. This closes out the 4-phase plan (BUILD_BACKLOG.md 0a) started in v2.35. None of this bears on the four go-live prerequisites (§38.6), which are unchanged from v2.6. |

## 58. `publish_new_version` Resource-Requirements Carry-Forward Fix (v2.39)

### 58.1 Origin

Found while verifying the v2.33 slot/duration reconciliation round (BUILD_BACKLOG.md §6, cited from baseline spec §52.4): `publish_new_version` (`ehr/routes/admin_scheduling.py`) already carried a type's color rules forward onto a freshly published `AppointmentTypeVersion`, but left `AppointmentTypeResourceRequirement` rows attached to the now-inactive previous version — so republishing a type that needs a room or other resource (e.g. "Contact Lens Evaluation/Check," which requires the Contact Lens Fitting Room) silently dropped that requirement from the new version, and the double-booking prevention the requirement exists for was quietly bypassed for every appointment booked against it afterward. The backlog item was left open behind an explicit design question: should every republish always carry resource requirements forward (mirroring color rules), or is there a legitimate workflow for changing them at publish time that a blanket carry-forward would break?

### 58.2 What changed

Resolved the open design question by inspection rather than by assumption: there is no admin UI anywhere in this app — not on the type edit/publish form, not on the type detail page, not as a standalone resource-requirements editor — to view or edit an `AppointmentTypeResourceRequirement` row at all, at publish time or otherwise. With no existing workflow that depends on requirements resetting on republish, a blanket carry-forward has no competing behavior to protect, so it gets the exact same treatment already established for color rules immediately above it in the same route: right after the color-rules copy loop in `publish_new_version`, a new loop copies every `AppointmentTypeResourceRequirement` row from the previous version onto the newly published one (`resource_id`, `resource_pool_code`, `required`, `offset_minutes`, `duration_minutes`). No schema change — this is a same-shape carry-forward fix, not a new capability. Requirements are still only editable via direct DB access, same as before this fix and same as color rules; a dedicated resource-requirements editor UI remains a documented future item, not part of this fix.

### 58.3 Verified

`python3 -m py_compile` on `ehr/routes/admin_scheduling.py` and `tests/test_smoke.py`. No migration/idempotency check applies — no schema changed. New Playwright test `test_publish_new_version_carries_resource_requirements_forward`: republishes "Contact Lens Evaluation/Check" via the real admin edit form (filling only the required Change Reason field, exactly as a staff user would for an otherwise-unremarkable republish), then confirms the *new* version still enforces its Contact Lens Fitting Room requirement — booking that new version for one provider makes the identical time slot disappear from a different provider's availability search for that same new version, proving the requirement carried forward rather than being silently dropped. The pre-existing `test_slot_granularity_and_resource_conflict_reconciliation` test's comment explaining why it avoided republishing "Contact Lens Evaluation/Check" (to dodge this exact gap) was updated now that the gap is closed. Full Playwright suite (53 tests, up from 52) re-run to confirm no regressions.

### 58.4 Explicitly not done

No admin UI added for viewing or editing resource requirements directly — requirements are still only manageable via direct DB access, both before and after this fix; that remains a documented future item. No change to how requirements are *created* (still only at initial type creation, via seed data or direct DB access) — this fix only addresses what happens to *existing* requirements when a type is republished. None of this bears on the four go-live prerequisites (§38.6), which are unchanged from v2.6.

**Version 2.39 change log (relative to v2.38) — `publish_new_version` Resource-Requirements Carry-Forward Fix:**
