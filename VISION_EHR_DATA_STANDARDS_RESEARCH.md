...
### 5.2 Anterior Segment & Ocular Surface Disease (Dry Eye) dashboard — **implemented in v2.11; renamed/split in v2.23**

**Note added in v2.23**: despite its name (both here and in the app), every field this section describes is dry-eye/OSD content — there was never a structural anterior-segment exam. In v2.23 this table/dashboard was renamed to `DryEyeAssessment` (no data change) and given its own Visit Focus chip ("Dry Eye / Ocular Surface Disease"), and a real, new structural `AnteriorSegmentAssessment` dashboard (conjunctiva/cornea/anterior chamber/iris/lens) was built alongside it — see baseline spec §42. This section is kept as-is below for historical accuracy about what v2.11 actually specified and built.
...
- **ICD-10 auto-suggestion.** A small lookup keyed on (diagnosis, laterality) — e.g. Myopia+OD → `H52.11`, +OS → `H52.12`, +OU → `H52.13` — to *suggest*, not force, a value into the existing free-text `diagnosis_codes` field. **Built in v2.15** exactly as scoped: extends the v2.12 composer function directly (no new endpoint), covering all six Refractive Assessment diagnoses (Emmetropia excluded — not billable). **Expanded in v2.25** (baseline spec §44) with verified codes for pterygium/pinguecula, three age-related cataract subtypes, dry eye syndrome, and cataract-extraction aftercare status, feeding the Anterior Segment/Dry Eye/Pre-Post-Op dashboards added in v2.23; `diagnosis_codes` now aggregates every active Visit Focus section's code(s), not just Refractive's. Still subject to the existing terminology-server deferral (§4.4): this remains a curated, hardcoded lookup — much narrower than real ICD-10 code-set integration — and should not be read as satisfying that larger item.
...
## 8. Gap analysis against a real visit-summary document export (v2.20)

**Source:** a real optometry "Visit Note" PDF export — a comprehensive visit-summary document (practice header, demographics, medications, ocular/social/medical/surgical history, allergies, a structured alerts checklist, review of systems, chief complaint/HPI, a detailed per-eye exam including pupils/motility/confrontation visual fields, diagnostic-imaging results, a numbered ICD-10-coded Impression/Plan with templated per-diagnosis counseling and dated follow-up addenda, and an e-signature block). The user asked for a component-by-component gap map against this application before any code was written, plus a plan to store documents per patient with no cross-patient contamination. Unlike §5/§6/§7's sources, this one is a real clinical output, not a requirements document — treat it as a structural reference for what a comparable document needs, not as a spec to match field-for-field.

**Already covered, at or beyond this source document's level:** demographics, allergies/medical/ocular/family-ocular history (free text), chief complaint, vision/IOP/basic slit-lamp/fundus (`EyeExam` core fields), and the five clinical dashboards (§5.1–§5.5) for refractive/dry-eye/glaucoma/binocular-vision/post-op detail. ICD-10 auto-suggestion (§7.2) and CSRF protection on the new routes below (baseline spec §37.7) come along for free.

**Built this round** — see baseline spec §39 for full detail:
- Generic per-patient document storage, with an explicit route-level cross-patient-contamination guard (§39.2).
- A first slice of a structured, longitudinal Problem List (diagnosis, ICD-10 code, laterality, templated counseling text, dated addenda linkable to a specific exam) — §39.3.

**Explicitly not built, newly tracked** (`BUILD_BACKLOG.md` §12): ~~pupil exam (size/reactivity/APD, light/dark/near)~~ **done, v2.21 — see baseline spec §40**; ~~motility and confrontation visual fields as structured per-eye data~~ **done, v2.22 — see baseline spec §41**; ~~conjunctiva/anterior-chamber/iris as discrete slit-lamp structures~~ **done, v2.23 — see baseline spec §42** (built together with lens/cataract grading as a full structural Anterior Segment dashboard, split out from the old "Anterior Segment / Dry Eye" dashboard); vitreous as a discrete fundus structure; structured review of systems; structured social history (alcohol/tobacco); diagnostic-imaging order + structured result tracking (fundus photos, OCT); e-signature/lock/sign-amend workflow and staff-present attribution beyond the single exam `Provider` (both pre-existing tracked gaps, spec §18.2 item 4/§37.6); and actual PDF rendering (this app has no PDF-generation library at all).

### 8.1 Pupil exam — **implemented in v2.21**

Ten new nullable columns on `EyeExam` (size at light/dark/near and reactivity per eye, plus a single APD finding field) — modeled as flat columns alongside Visual Acuity/Slit Lamp/Fundus, not a new Visit Focus dashboard, since pupils are core exam data present on nearly every visit rather than a diagnosis-driven specialty assessment. See baseline spec §40 for full detail.

### 8.2 Motility and confrontation visual fields — **implemented in v2.22**

Four new nullable columns on `EyeExam` (motility and confrontation visual field per eye) — same flat-column treatment as pupils (§8.1), distinct from the pre-existing `cover_test` field (ocular alignment/phoria-tropia, a different clinical concept from muscle movement or peripheral field testing). See baseline spec §41 for full detail.

### 8.3 Anterior Segment / Dry Eye split — **implemented in v2.23**

The user asked to separate "Anterior Segment" from "Dry Eye" in the New Exam Visit Focus setup. Investigation (prompted by this request, not by the original §8 gap analysis) confirmed §5.2's dashboard was, despite its name, entirely dry-eye/OSD content — this directly resolved §8's own "conjunctiva/anterior-chamber/iris as discrete slit-lamp structures" gap, one round earlier than its position in the unscoped list above would have suggested. §5.2's table/dashboard was renamed to `DryEyeAssessment` (no data change) and given its own Visit Focus chip; a real, new structural `AnteriorSegmentAssessment` dashboard (conjunctiva, cornea, anterior chamber, iris, lens/cataract grading) was built alongside it. See baseline spec §42 for full detail.
...
14. ~~Store documents per patient (with no cross-patient contamination) and start on the biggest missing piece from §8's gap analysis.~~ **Done, v2.20** — per-patient document storage and a first slice of the Problem List; see §8 and the baseline spec's §39. §8's remaining items (pupil exam, structured ROS/social history, diagnostic-imaging orders, e-signature/lock, PDF rendering) are newly tracked in `BUILD_BACKLOG.md` §12, not yet scoped for a build.
15. ~~Build pupil exam fields (§8's next-biggest item from `BUILD_BACKLOG.md` §12).~~ **Done, v2.21** — see §8.1 and the baseline spec's §40. §12's remaining items (motility/confrontation visual fields, discrete conjunctiva/AC/iris/vitreous structures, structured ROS/social history, diagnostic-imaging orders, e-signature/lock, PDF rendering) remain unscoped.
16. ~~Build motility and confrontation visual field fields (next item from `BUILD_BACKLOG.md` §12).~~ **Done, v2.22** — see §8.2 and the baseline spec's §41. §12's remaining items (discrete conjunctiva/AC/iris/vitreous structures, structured ROS/social history, diagnostic-imaging orders, e-signature/lock, PDF rendering) remain unscoped.
17. ~~Separate "Anterior Segment" from "Dry Eye" in the New Exam Visit Focus setup, thoroughly scoped (user request, not from the §8 gap-analysis queue).~~ **Done, v2.23** — see §8.3 and the baseline spec's §42. This also resolves §12's "discrete conjunctiva/AC/iris/vitreous structures" item (vitreous itself remains open — it's a fundus structure, not anterior segment). §12's remaining items (vitreous, structured ROS/social history, diagnostic-imaging orders, e-signature/lock, PDF rendering) remain unscoped.
