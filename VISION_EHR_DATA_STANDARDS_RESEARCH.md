...
**Explicitly not built, newly tracked** (`BUILD_BACKLOG.md` §12): ~~pupil exam (size/reactivity/APD, light/dark/near)~~ **done, v2.21 — see baseline spec §40**; ~~motility and confrontation visual fields as structured per-eye data~~ **done, v2.22 — see baseline spec §41**; conjunctiva/anterior-chamber/iris as discrete slit-lamp structures; vitreous as a discrete fundus structure; structured review of systems; structured social history (alcohol/tobacco); diagnostic-imaging order + structured result tracking (fundus photos, OCT); e-signature/lock/sign-amend workflow and staff-present attribution beyond the single exam `Provider` (both pre-existing tracked gaps, spec §18.2 item 4/§37.6); and actual PDF rendering (this app has no PDF-generation library at all).
